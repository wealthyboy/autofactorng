...
abstract class Table extends Controller
{
        ...
        try {
            ...
            return $data =  [
                ...
                'meta' => [
                    ...
                    'selected' => method_exists($builder, 'selected') ?  $builder->selected($collections) : null
                ],
                'unique' =>  $this->unique(),
                'routes' => $this->routes()
            ];
    ...
    protected function buildSearch(Builder $builder, Request $request)
    {
        ...
        $query =  $this->builder()->where(function (Builder $query) use ($request, $term) {
            ...
            if (in_array('name', $this->getDatabaseColumnNames()) &&  in_array('last_name', $this->getDatabaseColumnNames())) {
