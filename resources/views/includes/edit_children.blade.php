            ...
            <div class="d-flex">
                <div class="form-check ">
                    <label  class="custom-control-label" for="{{ $ob->slug }}-{{ $ob->id }}">
                        <input  class="form-check-input  car-models {{$url}} {{ $obj->slug }}  {{ $obj->name == 'Spare Parts' || $obj->name == 'Servicing Parts'  ? 'no-validation' : '' }}" data-slug="{{ $ob->slug }}"  data-name="{{ $ob->name }}" value="{{ $obj->id }}" {{ $helper->check($collections, $ob->id) }} type="checkbox" id="{{ $ob->slug }}-{{ $ob->id }}" name="{{$name}}[]" >
