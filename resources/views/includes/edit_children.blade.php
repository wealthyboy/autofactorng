            ...
            <div class="d-flex">
                <div class="form-check ">
                    <label  class="custom-control-label" for="{{ $ob->slug }}-{{ $ob->id }}">
                        <input  class="form-check-input  car-models {{$url}} {{ $obj->slug }}  {{ $obj->name == 'Spare Parts' || $obj->name == 'Servicing Parts'  ? 'no-validation' : '' }}"  data-name="{{ $ob->slug }}" value="{{ $ob->id }}" {{ $helper->check($collections, $ob->id) }} type="checkbox" id="{{ $ob->name }}-{{ $ob->id }}" name="{{$name}}[]" >
