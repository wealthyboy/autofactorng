        ...
        <div class="d-flex">
            <div class="form-check">
                <label class="custom-control-label " for="{{ $ob->slug }}-{{ $engine->id }}">
                    <input class="form-check-input  engine-{{ $ob->slug }}" value="{{ $engine->id }}" type="checkbox" id="{{ $ob->slug }}-{{ $engine->id }}" name="engine_id[{{ $ob->id }}][]" data-slug="{{ $ob->slug }}" data-name="{{ $ob->name }}">
