<div id="load-products" class="row">
    ...
    <div class="col-6  {{ $no_attr ? 'col-md-3' : 'col-md-4' }}">
        <div title="{{ $product->meta_title }}" class="product-default inner-quickview inner-icon">
            <figure>
                <a href="{{ $product->link }}">
                    <img src="{{ $product->image_to_show_m }}" alt="{{ $product->meta_title }}" />
                </a>
                ...
                <div data-iswishlist="{{ $product->is_wishlist ? 1: 0}}" data-pid="{{ $product->default_variation_id }}" class="btn-icon-group">
                    ...
                    <div data-toggle="modal" data-target="#login-modal" data-iswishlist="{{ $product->is_wishlist ? 1: 0}}" data-pid="0" class="btn-icon-group">
                        ...
                        <svg class="product-wishlist-icon  {{ $product->is_wishlist ? 'd-none': ''}}">
                            <use xlink:href="#iconStar">
                                <symbol data-icon-id="star" id="iconStar">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                        <path fill-rule="evenodd" d="M12 2l2.868 6.922L22 9.844l-5.11 4.804L18.225 22 12 18.322 5.776 22l1.333-7.352L2 9.844l7.132-.922L12 2zm-1.49 8.816l-3.976.513 2.733 2.57-.745 4.11L12 15.955l3.478 2.056-.745-4.111 2.733-2.57-3.975-.514L12 7.219l-1.49 3.598z"></path>
                                    </svg>
                                </symbol>
                            </use>
                        </svg>
