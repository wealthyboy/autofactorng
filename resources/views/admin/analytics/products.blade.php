...
<div class="row">
    ...
    <div class="col-lg-4 mb-4"><div class="card h-100"><div class="card-header pb-0"><h6>Low-stock products updated in period</h6></div><div class="card-body px-0 pb-2"><div class="table-responsive"><table class="table align-items-center mb-0"><thead><tr><th>Product</th><th class="text-end">Stock</th></tr></thead><tbody>
...
<div class="row">
    <div class="col-lg-6 mb-4"><div class="card h-100"><div class="card-header pb-0"><h6>Fast-moving products</h6><p class="text-xs text-secondary">More than 5 units sold in selected period</p></div><div class="card-body px-0"><table class="table mb-0"><tbody>@forelse($fastProducts as $product)<tr><td class="px-4 text-sm">{{ $product->name }}</td><td class="text-end px-4">{{ number_format($product->units) }} units</td></tr>@empty<tr><td class="text-center py-4">No fast-moving products.</td></tr>@endforelse</tbody></table></div></div></div>
    <div class="col-lg-6 mb-4"><div class="card h-100"><div class="card-header pb-0"><h6>Slow-moving products</h6><p class="text-xs text-secondary">No sale in selected period</p></div><div class="card-body px-0"><table class="table mb-0"><tbody>@forelse($slowProducts as $product)<tr><td class="px-4"><span class="text-sm">{{ $product->name }}</span><div class="text-xs text-secondary">{{ $product->sku ?: 'No SKU' }}</div></td></tr>@empty<tr><td class="text-center py-4">No slow-moving products.</td></tr>@endforelse</tbody></table></div></div></div>
</div>
@endsection
