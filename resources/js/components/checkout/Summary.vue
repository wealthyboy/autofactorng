<template>
  ...
  <p class="pt-3 pb-1 d-flex justify-content-between">
    ...
    <span class="bold float-right">
      <span class="currencySymbol">{{ cart_meta.sub_total }}</span>
    </span>
  </p>
  <p class="border-top border-bottom pb-3 pt-3  d-flex justify-content-between"><span class="bold">Shipping</span> <span class="bold float-right"><small> {{ prices.ship_price }}</small></span></p>

  <p
    v-if=" prices.heavy_item_price"
    class="border-top border-bottom pb-3 pt-3  d-flex justify-content-between"
  ><span class="bold">Heavy/Large Items Charge</span> <span class="bold float-right"><small> {{ prices.heavy_item_price }}</small></span></p>
