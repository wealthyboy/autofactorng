    ...
    <div class="product-default inner-quickview inner-icon">
      <figure>
        ...
        <div class="label-group">
          ...
          >-{{ product.default_percentage_off }}%</div>
        </div>
        <div class="btn-icon-group">
          <button
            ...
            :style=" [wishlistIsActive ? wishA : '']"
            class="btn-icon btn-add-cart"
            @click="addToWishList(product.default_variation_id)"
          ><i
              :style=" [wishlistIsActive ? wishA : '']"
              class="icon-heart"
            ></i></button>
      ...
      <div class="product-details text-center">
        <div class="mx-auto">
          <div
            ...
            class="justify-content-center d-flex mb-1"
          >
            <div
              v-for="color in product.colours"
              :style="{ 'background-color': color.color_code }"
              style="border:1px solid #222; height: 15px; width: 15px; border-radius: 50%;"
              class="mr-1"
            >
            </div>
            <symbol
              data-icon-id="star"
              data-icon-set="farfetch-2020"
              id="iconLoaded-star"
            ><svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
...
export default {
  ...
  computed: {
    ...
    rowClass: function () {
      return Object.keys(this.category)[0] == "no_attributes"
        ? "col-lg-3 col-md-3"
        : "col-lg-4 col-md-4";
    },
    wishlistIsActive: function () {
      if (this.wishlist.length) {
        if (
          this.wishlist.some(
            (wishlist) =>
              wishlist.product_variation.id ===
              this.product.default_variation_id
          )
        ) {
          return "#222";
        }
  ...
  methods: {
    ...
    addToWishList: function (product_variation_id) {
      this.addProductToWishList({
        ...
        if (
          this.wishlist.some(
            (wishlist) => wishlist.product_variation.id === product_variation_id
          )
        ) {
